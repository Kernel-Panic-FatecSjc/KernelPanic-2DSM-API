...
function App() {
  ...
  const clientesPendentes = clientes.filter(c => c.status.toLowerCase().includes('pendente')).length;
  const clientesAtivos = clientes.filter(c => c.status.toLowerCase().includes('ativo')).length;
  ...
  const getStatusClass = (status) => {
    ...
    if (statusLower.includes('concluido') || statusLower.includes('concluído')) return 'concluido';
    return 'pendente';
  };
  ...
  return (
    <div className={styles.conteudo}>
      <div className={styles['container-gestao']}>
        {/* HEADER com numeros de clientes */}
        <div className={styles.header}>
          <div className={styles.headerLeft}>
            <h1 className={styles.titulo}>Gestão de clientes</h1>
            <p className={styles.subtitulo}>Gerencie seus clientes e acompanhe vendas</p>
          </div>
          <div className={styles.headerRight}>
            <div className={styles.statCard}>
              <p className={`${styles.statNumber} ${styles.total}`}>{totalClientes}</p>
              <p className={styles.statLabel}>Total Clientes</p>
            </div>
            <div className={styles.statCard}>
              <p className={`${styles.statNumber} ${styles.pendente}`}>{clientesPendentes}</p>
              <p className={styles.statLabel}>Pendentes</p>
            </div>
            <div className={styles.statCard}>
              <p className={`${styles.statNumber} ${styles.ativo}`}>{clientesAtivos}</p>
          ...
          {sortedClients.map(client => (
            <div key={client.id} className={styles.clientCard}>
              <div className={styles.cardHeader}>
                ...
                <span className={`${styles.statusBadge} ${styles[getStatusClass(client.status)]}`}>
                  {client.status}
                </span>
              </div>
              ...
              <div className={styles.cardDetails}>
                <div className={styles.detailItem}>
                  <span className={styles.detailLabel}>Última Interação</span>
                  <span className={styles.detailValue}>
                    {new Date(client.ultimaInteracao).toLocaleDateString('pt-BR')}
                  </span>
                </div>
                <div className={styles.detailItem}>
                  <span className={styles.detailLabel}>Vendas</span>
                  <span className={styles.detailValue}>{formatCurrency(client.vendas)}</span>
                </div>
              </div>
        ...
        <div className={styles.modalAddOverlay}>
          <div className={styles.modalAddContent}>
            ...
            <form onSubmit={handleAdd}>
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>Segmento de atuação</label>
                <input
                  type="text"
                  className={styles.formInput}
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>Status da venda</label>
                <select
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>Departamento responsável</label>
                <input
                  type="text"
                  className={styles.formInput}
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>Tipo de contato</label>
                <select
                  className={styles.formSelect}
                  value={newClient.tipoContato}
                  onChange={(e) => setNewClient({ ...newClient, tipoContato: e.target.value })}
                >
                  <option value="telefone">Telefone</option>
                  <option value="celular">Celular</option>
                  <option value="email">E-mail</option>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  onChange={(e) => setNewClient({ ...newClient, contatoValor: e.target.value })}
                  placeholder={newClient.tipoContato === 'email' ? 'exemplo@email.com' : '(00) 00000-0000'}
                />
              </div>

              <div className={styles.buttonGroup}>
                <button type="button" className={styles.btnCancel} onClick={() => setShowModalAdd(false)}>
                  Cancelar
                </button>
